...
import type { PageItem, SourceFile, PageFormat, PageOrientation, ImageFilterType } from '../types';
...
interface PreviewModalProps {
  ...
  onUpdatePageFilter?: (pageId: string, filter: ImageFilterType) => void;
}
...
export const PreviewModal: React.FC<PreviewModalProps> = ({
  ...
  }, [currentPage?.layout, currentPage?.rotation, currentPage?.crop, currentPage?.filter, showMarginGuides, currentPage]);
    ...
    <div className="fixed inset-0 z-50 bg-black/92 backdrop-blur-md flex flex-col select-none animate-fade-in text-white font-mono">
      <div className="h-14 px-4 sm:px-6 bg-black border-b border-neutral-800 flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {onUpdatePageFilter && (
            <div className="flex items-center border border-neutral-700 bg-neutral-900">
              <span className="px-1.5 text-neutral-400"><Wand2 size={12} /></span>
              <select
                value={currentPage.filter || 'none'}
                onChange={(e) => onUpdatePageFilter(currentPage.id, e.target.value as ImageFilterType)}
                className="px-1.5 py-1 bg-neutral-900 text-xs text-white uppercase focus:outline-none"
              >
                <option value="none">Normal</option>
                <option value="clean-scan">Clean Scan (White BG)</option>
          ...
          <button onClick={() => setShowMarginGuides(!showMarginGuides)} className={`p-1.5 border border-neutral-700 text-xs transition-colors hidden lg:flex items-center gap-1 ${showMarginGuides ? 'bg-neutral-800 text-white' : 'bg-neutral-900 text-neutral-400'}`} title="Toggle Margin Guides">
            <Grid size={12} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1 border border-neutral-700 bg-neutral-900 p-0.5 text-xs">
            <button onClick={handleZoomOut} className="p-1 hover:bg-white hover:text-black transition-colors" title="Zoom Out (-)"><ZoomOut size={13} /></button>
            <span className="px-1.5 text-[11px] font-bold min-w-10 text-center">{Math.round(zoom * 100)}%</span>
            <button onClick={handleZoomIn} className="p-1 hover:bg-white hover:text-black transition-colors" title="Zoom In (+)"><ZoomIn size={13} /></button>
            <button onClick={handleZoomReset} className="p-1 hover:bg-white hover:text-black transition-colors border-l border-neutral-700" title="Reset Zoom (0)"><Maximize size={13} /></button>
          </div>

          <div className="flex items-center border border-neutral-700 bg-neutral-900">
            <button onClick={() => onRotateCcw(currentPage.id)} className="p-1.5 hover:bg-white hover:text-black transition-colors" title="Rotate Left (-90°)"><RotateCcw size={13} /></button>
            <button onClick={() => onRotateCw(currentPage.id)} className="p-1.5 hover:bg-white hover:text-black border-l border-neutral-700 transition-colors" title="Rotate Right (+90°)"><RotateCw size={13} /></button>
          </div>

          <button onClick={() => { onClose(); onOpenCrop(currentPage); }} className="p-1.5 border border-neutral-700 bg-neutral-900 hover:bg-white hover:text-black transition-colors text-xs flex items-center gap-1" title="Crop page"><CropIcon size={13} /><span className="hidden sm:inline">CROP</span></button>

          <button onClick={() => { onDelete(currentPage.id); if (pages.length <= 1) onClose(); else if (currentIndex >= pages.length - 1) setCurrentIndex(pages.length - 2); }} className="p-1.5 border border-neutral-700 bg-neutral-900 hover:bg-red-600 transition-colors" title="Delete page"><Trash2 size={13} /></button>

          <button onClick={onClose} className="p-1.5 ml-2 bg-white text-black hover:bg-neutral-200 transition-colors font-bold" title="Close preview (Esc)"><X size={15} /></button>
        </div>
      </div>
      ...
      <div className="flex-1 relative flex items-center justify-center p-4 sm:p-8 overflow-auto bg-neutral-900/90">
        {currentIndex > 0 && (
          <button onClick={goToPrev} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-black/80 hover:bg-white hover:text-black border border-white/30 flex items-center justify-center transition-all cursor-pointer shadow-xl" title="Previous Page (←)">
            <ChevronLeft size={24} />
          </button>
        )}

        {currentIndex < pages.length - 1 && (
          <button onClick={goToNext} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-black/80 hover:bg-white hover:text-black border border-white/30 flex items-center justify-center transition-all cursor-pointer shadow-xl" title="Next Page (→)">
